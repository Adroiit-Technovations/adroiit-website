  ...
  return (
    <div className={`${bricolage.className} bg-black text-white`}>
              ...
              <div className="space-y-8">
                <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={0}>
                  <h1 className="text-4xl md:text-3xl font-black leading-tight tracking-tight">
                    Mastering Precision in <span className="text-[#8c52ff]">Mechanical CAD</span>
                  </h1>
                ...
                <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={2} className="flex flex-col sm:flex-row items-center gap-4">
                  <Link href="/contact" className="group inline-flex items-center gap-3 bg-[#8c52ff] text-white px-8 py-4 rounded-full font-bold hover:bg-purple-600 transition-all shadow-[0_0_20px_rgba(147,51,234,0.3)]">
      ...
      <section className="bg-white text-black py-8 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-6">
            <h2 className="text-3xl md:text-2xl font-black text-[#8c52ff] tracking-tight mb-2">What we offer</h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: <Box className="text-[#8c52ff]" size={28} />, title: "3D Modeling", desc: "Accurate parametric and surface models for product design and development." },
              { icon: <Layers className="text-[#8c52ff]" size={28} />, title: "2D Drafting", desc: "Detailed engineering drawings with GD&T and manufacturing standards." },
              { icon: <Cog className="text-[#8c52ff]" size={28} />, title: "Assembly Design", desc: "Complex assemblies with motion, fit, and interference analysis." },
              { icon: <Wrench className="text-[#8c52ff]" size={28} />, title: "Reverse Engineering", desc: "Convert physical parts into precise digital CAD models." },
              { icon: <Factory className="text-[#8c52ff]" size={28} />, title: "Manufacturing Support", desc: "Designs optimized for CNC, injection molding, and fabrication." },
              { icon: <Cpu className="text-[#8c52ff]" size={28} />, title: "Simulation Ready Models", desc: "CAD prepared for FEA, CFD, and real-world testing workflows." },
            ].map((item, i) => (
              <motion.div key={i} whileHover={{ scale: 1.02 }} className="p-6 rounded-2xl border border-gray-200 bg-gray-50/50 hover:bg-white hover:shadow-xl hover:border-purple-500 transition-all duration-300 flex flex-col items-center text-center group">
                ...
                <h3 className="text-lg font-bold mb-2 text-[#8c52ff]">{item.title}</h3>
      ...
      <section className="max-w-7xl mx-auto px-6 py-8 relative overflow-hidden">
        <div className="text-center mb-6">
          <h2 className="text-3xl md:text-2xl font-bold text-[#8c52ff]">Our workflow</h2>
            ...
            {workflowSteps.map((step, i) => (
              <div key={i} className="flex flex-col items-center group">
                <div className="relative mb-4">
                  <motion.div whileHover={{ scale: 1.1, rotate: 5 }} className="w-20 h-20 rounded-2xl bg-neutral-900 border border-purple-500/30 flex items-center justify-center relative z-10 group-hover:border-[#8c52ff] transition-all shadow-[0_0_30px_rgba(140,82,255,0.1)]">
                    <div className="text-[#8c52ff] group-hover:text-white transition-colors">{step.icon}</div>
                    <div className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-[#8c52ff] text-white text-xs font-bold flex items-center justify-center border-4 border-black">{i + 1}</div>
                  </motion.div>
                </div>
                <div className="text-center px-4">
                  <h3 className="text-xl font-bold text-[#8c52ff] mb-3 group-hover:text-purple-600 transition-colors">{step.title}</h3>
      ...
      <section className="py-8 px-6 bg-gray-50 relative overflow-hidden">
        <div className="max-w-[1440px] mx-auto relative z-10">
          <div className="text-center mb-6">
            <h2 className="text-3xl md:text-2xl font-black text-[#8c52ff] mb-4">Why choose Adroiit Technovations</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
            {whyChooseItems.map((item, i) => (
              <div key={i} className="group p-5 rounded-2xl bg-white border border-gray-200 hover:border-purple-300 hover:shadow-lg transition-all duration-300 flex flex-col items-center text-center justify-center min-h-[200px]">
                <div className="w-12 h-12 rounded-xl bg-purple-50 flex items-center justify-center text-[#8c52ff] mb-4 group-hover:scale-110 transition-transform">{item.icon}</div>
                <h3 className="text-sm font-bold text-[#8c52ff] mb-2 transition-colors tracking-wide">
                  {item.title}
                </h3>
      ...
      <section className="relative py-8 px-6 bg-gray-100">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl md:text-2xl font-extrabold text-[#8c52ff]">Have a design idea? <span className="text-gray-900">Let's transform!</span></h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-5">
            <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-2 px-8 py-3.5 bg-[#8c52ff] text-white font-bold rounded-full transition-all shadow-lg hover:bg-purple-600 hover:scale-105 active:scale-95">
            ...
            <div className="flex items-center gap-3 px-5 py-3 rounded-full bg-gray-100 border border-gray-200">
              <CheckCircle className="text-[#8c52ff]" size={18} />
