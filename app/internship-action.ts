...
export async function handleInternshipApplication(formData: FormData) {
  try {
    // Convert basic text fields to an object
    const data = Object.fromEntries(formData.entries());
    
    // Extract file attachments (Photo and Resume)
    ...
    // Capture all selected skill checkboxes
    const skills = formData.getAll("skills").join(", ");

    const { error } = await resend.emails.send({
      from: 'Adroiit Career <onboarding@resend.dev>',
      to: ['adroiit.technovations@gmail.com'],
      subject: `Internship Application: ${data.fullName} - ${data.domain}`,
      ...
      html: `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #1a1a1a; max-width: 600px; border: 1px solid #e0e0e0; padding: 30px; border-radius: 15px;">
          <h2 style="color: #8c52ff; border-bottom: 2px solid #f0f0f0; padding-bottom: 10px; margin-top: 0;">New Application Received</h2>
          <p style="font-size: 14px; color: #666;"><b>Applied For:</b> <span style="color: #8c52ff; font-weight: bold;">${data.domain}</span></p>
          ...
          <p style="margin: 5px 0;"><b>WhatsApp:</b> ${data.whatsapp}</p>
          ...
          <p style="margin: 5px 0;"><b>Current Year:</b> ${data.yearOfStudy}</p>
          <p style="margin: 5px 0;"><b>Expected Graduation:</b> ${data.passingYear}</p>
          <p style="margin: 5px 0;"><b>LinkedIn Profile:</b> <a href="${data.linkedin}" style="color: #0077b5;">${data.linkedin}</a></p>
          ...
          <p style="margin: 5px 0;"><b>Selected Skills:</b> ${skills || 'None selected'}</p>
          ...
          <div style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #eee; font-size: 11px; color: #999; text-align: center;">
            This is an automated notification from the Adroiit Internship Portal.
          </div>
        </div>
      `,
    });
